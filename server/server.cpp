...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/23 19:13:01 by aqadil            #+#    #+#             */
...
DIY_server::DIY_server(std::vector<Config> SV_d) : SV_data(SV_d)
{
    this->fd_sk_num = 0;
...
void DIY_server::init_sockets()
{
    ...
    {
        if (std::find(vec_ports.begin(), vec_ports.end(), SV_data[i].getPort()[0]) != vec_ports.end())
        {
            continue;
            i++;
        }
        this->sk_list.push_back(DIY_socket());
        if (!(this->sk_list[i].Launch_sk(SV_data[i].getPort()[0], SV_data[i].getHost())))
        {
            continue;
            i++;
        }
        if (sk_list[i].listen_sk() < 0)
        {
            ...
            vec_ports.push_back(SV_data[i].getPort()[0]);
        i++;
    }
}
...
int DIY_server::list_polls()
{
    ...
    while (i < this->sk_list.size())
    {
        ...
        poll_fds.fd = id_skfd;
        poll_fds.events = POLLIN | POLLOUT | POLLHUP;
        poll_fds.revents = 0;
...
void DIY_server::Manager_I(int fd_plfdlist, int curr_req)
{
    ...
    else
    {
        ...
        else if(num_data_readed > 0)
        {
            ...
            {
                manage_RR req(this->RD_sock_accepted[index].get_rd_request(), SV_data, env);
                this->sv_request = req;
                // exit (0);
            }
        }
...
void DIY_server::Manager_O(int fd_plfdlist, int curr_req)
{
    ...
    if(this->RD_sock_accepted[index].get_rd_rdgotreq() == true)
    {
        ...
        std::string resp;
        this->RD_sock_accepted[index].set_rd_request(sv_request.get_res());
        ...
        if(num_data_sended < 1 || this->RD_sock_accepted[index].get_rd_size() == this->RD_sock_accepted[index].get_rd_numdata_sended())
        {
            std::cout << "This socket has been disconnected : " << fd_plfdlist << std::endl;
            close(fd_plfdlist);
            this->poll_list.erase(this->poll_list.begin() + curr_req);
